...
def mixedfraction(num, den):
    ...
    '''
    #Numerator floor division denominator returns the whole number
    #Numerator modulo denominator returns the remaining numerator
...
def iscyclops(n):
    ...
    if strN[len(strN)//2] == "0": 
        return True if "0" not in strN.replace("0", "", 1) else False
    return False
...
def paritypartition(items):
    ...
    for e in evens: new_list.append(e)
    for o in odds: new_list.append(o)
