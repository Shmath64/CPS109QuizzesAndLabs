...
def inversions(items):
    ...
    for i in range(len(items)): 
        #count number of later-appearing 'smaller' (as determined by '<') elements
...
def increasing(items, strict):   
    ...
    if strict:
        for i in range(length):
            if i != 0: #don't worry about the first
                if items[i] != items[i-1] + 1: # is the current item 1 more than the previous?
                    return False
    else:
        for i in range(length):
            if i != 0: #don't worry about the first
                if items[i] < items[i-1]: # is the current item not less than the previous?
                    return False
    return True
